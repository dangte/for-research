...
This repository is a **private backup**. Skills come from mixed sources; this file
records what is known so that licensing stays honest, especially before any future
decision to make the repository public.

| Skill | Authorship | License status |
|---|---|---|
...
## Before going public
1. Confirm the author/source of every "UNCONFIRMED" skill above.
2. For any skill authored by a third party, keep its original license + attribution
   (as done for `humanizer`), or remove it.
3. Only then update `LICENSE`/this file and change repository visibility.

## Private-only components
- `clean-user-facing-text` (watermark/Unicode cleaner) — included as a **personal backup only**.
  Its license/origin is unconfirmed (see table above). **Do not** make this repository public
  without first establishing this skill's license, or remove it beforehand.
